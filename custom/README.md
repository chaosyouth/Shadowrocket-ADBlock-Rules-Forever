...
配置保留原文件名与用途；含代理策略的版本使用当前本机配置中的 `CF最优`。
订阅名称使用当前的 `EDGETUNNEL.CHAOSYOUTH.COM`，添加下表中的配置地址并使用配置。
[reference.json](reference.json) 只保存当前配置中的一个 CF 优选组，自动优选每 60 秒测试一次。只调整通用 PROXY 引用，其他应用组默认选择、DNS、IPv6、Hosts、分流及重写设置均保持上游配置。
每个文件均包含指向自身 GitHub Pages 下载地址的 `update-url`，每日生成时自动维护。
